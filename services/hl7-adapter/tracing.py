...
def extract_kafka_context(headers: list[tuple[str, bytes]] | None) -> Context:
    ...
    por empi-service, ver app/tracing.py:inject_kafka_headers)."""
    carrier = {k: v.decode("utf-8") for k, v in (headers or [])}
    return propagate.extract(carrier)


def inject_http_headers() -> dict[str, str]:
    """Contexto de la traza activa como headers HTTP salientes (llamada al HCE mock)."""
